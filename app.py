...
Backend Flask – Authentification + formations prédéfinies
"""
import os
import re
import uuid
...
def generer_pptx(participant, session_data, formation, pptx_sortie):
    ...
    for shape in slide.shapes:
            ...
            if rpps_requis and rpps:
                for para in shape.text_frame.paragraphs:
                    ...
                    if 'Optionnelle_Numero_RPPS' in t or ('Optionnelle_' in t and 'RPPS' in t.upper()):
                        # Remplacer le bloc entier par "N°RPPS : <numero>"
                        new_t = re.sub(
        ...
        # Nettoyage final : supprimer placeholders résiduels
        for para in shape.text_frame.paragraphs:
            t = _texte_para(para)
...
def generer():
    ...
    if rpps_requis:
        ...
        if manquants:
            return jsonify({'error': f"RPPS manquant pour : {', '.join(manquants)}"}), 400
    ...
    for p in participants:
        ...
        try:
            ...
            entry = {'nom': nom, 'pptx': pptx_path.name}
            if pdf_path:
